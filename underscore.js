...
(function() {
  ...
  // 建立根对象，在浏览器中是`window` (`self`)，在服务器上是`global`，在一些虚拟机上是`this`。
  // 对于`WebWorker`的支持，我们使用`self`而不是`window`。
  ...
  // 在 minified（不是 gzziped）版本中节省字节
  var ArrayProto = Array.prototype, ObjProto = Object.prototype;
  var SymbolProto = typeof Symbol !== 'undefined' ? Symbol.prototype : null;

  // Create quick reference variables for speed access to core prototypes.
  // 创建能够快速访问那些核心原型的快速引用变量。
  ...
  // are declared here.
  // 所有的我们希望用到的 **ECMAScript 5** 原生函数的实现，都在这里声明了
  var nativeIsArray = Array.isArray,
      nativeKeys = Object.keys,
      nativeCreate = Object.create;

  // Naked function reference for surrogate-prototype-swapping.
  // 空函数，参考 surrogate-prototype-swapping
  // surrogate-prototype-swapping：一个将被用作构造函数的函数，以便将原型添加到新对象
  var Ctor = function(){};
  ...
  // 导出 **Node.js** 的 Underscore 对象，具有向后兼容它们的旧模块API的特性
  // 如果在浏览器环境，添加` _` 为一个全局对象
  // `nodeType` 是用来确认 `module` 和 `exports` 不是 HTML 元素
  ...
  // 内部函数
  // 生成可以被应用于一个集合中任意元素的回调函数
  // 返回被需要的结果 —— 任意‘身份’，一个回调函数，一个属性匹配器或者是一个属性访问器
  ...
  // 回调生成函数的外部包装函数
  // 如果用户需要其他谓词/迭代使用样式，可以自定义 `_.iteratee`
  // 这个抽象函数隐藏了内部参数 argCount
...
    // 不是的话只需返回一个对象（通过对象字面量），因为这里没有一个有效的对象赖以继承
  ...
  // 对集合中的每个元素执行 iteratee 函数，将返回值存放在一个新数组里，并返回此新数组
  ...
  // or `foldl`.
  // **Reduce** 接受一个集合，返回一个单一的结果
  // 接受四个参数：list | iteratee | memo | context
  // iteratee 也接受四个参数：memo | value | index | list
  _.reduce = _.foldl = _.inject = createReduce(1);

  // The right-associative version of reduce, also known as `foldr`.
  // **reduceRight** 是 **Reduce** 从右向左遍历的版本
  ...
  // 用于创建 findIndex 和 findLastIndex 的构建函数
  ...
  // 对于数组或者类数组，从左向右遍历，返回第一个满足条件的 index
  _.findIndex = createPredicateIndexFinder(1);

	// 对于数组或者类数组，从右向左遍历，返回第一个满足条件的 index
  ...
  // 对于传入的 predicate 返回一个相反的版本
  // e.g 原先 predicate 返回 true， _.negate 返回的函数就会返回 false
  _.negate = function(predicate) {
    return function() {
			// 这里使用 apply 是因为无法知道实参个数
			// 这里传入 this 是因为 this 指针在外部可能会变化，如果不加的话外部的 context 会传不进来
			// Refer：https://stackoverflow.com/questions/22396582/why-does-negate-uses-apply-with-this-parameter-in-underscore-js
  ...
  _.findKey = function(obj, predicate, context) {
		// predicate 接受三个参数，value | index | obj
    predicate = cb(predicate, context);
  ...
  // 添加了 'chain' 函数。它会帮你链接到 Underscore 对象
  ...
  // 如果 Underscore 被当做一个函数调用，它会返回一个被包裹的对象，该对象包含所有的 underscore 方法
  // 被包裹的对象互相之间是连通的，可以链式调用
  ...
  _.prototype.toString = function() {
	...
	var a = _.reduce([1,2,3], function(memo, num) {return memo+num});
	var b = _.reduce({a:1}, function(memo, num) {return memo+num});
